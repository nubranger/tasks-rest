...
if (array_key_exists("sessionid", $_GET)) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
        ...
        if (!isset($jsonData->refresh_token) || strlen($jsonData->refresh_token) < 1) {
            ...
            $response->setHttpStatusCode(400);
            $response->setSuccess(false);
            (!isset($jsonData->refresh_token) ? $response->setMessages("Refresh Token not supplied") : false);
            (strlen($jsonData->refresh_token) < 1 ? $response->setMessages("Refresh Token cannot be blank") : false);
            $response->send();
            exit;
        }
        ...
        try {
            ...
            if ($rowCount === 0) {
                ...
                response(
                    401,
                    false,
                    "Access Token or Refresh Token is incorrect for session id",
                    false,
                    []
                );
            }
            ...
            if ($rowCount === 0) {
                ...
                response(
                    401,
                    false,
                    "Access token could not be refreshed - please log in again",
                    false,
                    []
                );
            }
            ...
            response(
                500,
                false,
                "There was an issue refreshing access token - please log in again",
                false,
                []
            );
        }
